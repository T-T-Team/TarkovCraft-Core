...
public interface EntityInteraction {
    ...
    ResourceLocation SHARED_ERROR_IDENTIFIER = TarkovCraftCore.createIdentifier("shared");
    ...
    final class Type<T extends EntityInteraction> {
        ...
        public static <T extends EntityInteraction> Builder<T> builder(ResourceLocation identifier) {
            return new Builder<>(Objects.requireNonNull(identifier, "Identifier cannot be null"));
        }

        public static <T extends EntityInteraction> Builder<T> singletonBuilder(ResourceLocation identifier, T interactionInstance) {
            return Type.<T>builder(identifier)
                    .withFactory(ctx -> interactionInstance);
        }
