...
public class DebugLayer implements GuiLayer {
    ...
    public void render(GuiGraphicsExtractor guiGraphics, DeltaTracker deltaTracker) {
        ...
        guiGraphics.text(font, "Move Stamina Component: " + EnergySystem.MOVEMENT_STAMINA, 5, y(), 0xFFFFFFFF);
        guiGraphics.text(font, "Arm Stamina Component: " + EnergySystem.ARM_STAMINA, 5, y(), 0xFFFFFFFF);
        renderEnergyStats(font, guiGraphics, player);

        renderCoreAttributeData(font, guiGraphics, attributeData);
    }

    private void renderEnergyStats(Font font, GuiGraphicsExtractor graphics, Player player) {
    ...
    private void renderCoreAttributeData(Font font, GuiGraphicsExtractor graphics, EntityAttributeData attributeData) {
        graphics.text(font, "Core Attributes", 5, y(), 0xFFFFFFFF);
        float forgetRate = attributeData.getAttribute(CoreAttributes.MEMORY_FORGET_TIME_MULTIPLIER).floatValue();
        graphics.text(font, String.format(Locale.ROOT, "Memory forget rate: %.2f", forgetRate), 5, y(), 0xFFFFFFFF);
        float forgetAmount = attributeData.getAttribute(CoreAttributes.MEMORY_FORGET_AMOUNT_MULTIPLIER).floatValue();
        graphics.text(font, String.format(Locale.ROOT, "Memory forget amount: %.2f", forgetAmount), 5, y(), 0xFFFFFFFF);
        ++line;
    }

    private void renderStaminaInfo(Font font, GuiGraphicsExtractor graphics, Player player, String name, StaminaComponent component) {
        if (component.isOverlayVisible(player)) {
            ...
            graphics.text(font, value, 5, y(), 0xFFFFFFFF);
        }
    }
