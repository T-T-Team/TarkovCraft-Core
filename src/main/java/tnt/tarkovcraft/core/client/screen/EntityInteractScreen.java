...
public final class EntityInteractScreen extends Screen {
    ...
    private void onInteractionStarted(EntityInteraction.Type<?> interaction) {
        ...
        PacketDistributor.sendToServer(C2S_RequestInteractionState.start(interaction, this.entity, initiationTime));
    }
    ...
    private void onInteractCancelled(EntityInteraction.Type<?> interaction) {
        ...
        PacketDistributor.sendToServer(C2S_RequestInteractionState.cancel(interaction, this.entity));
    }
    ...
    private void interactionCompleteCallback(EntityInteraction.Type<?> interaction) {
        ...
        PacketDistributor.sendToServer(C2S_RequestInteractionState.finish(interaction, this.entity));
        this.minecraft.setScreen(null);
    }
    ...
    public void removed() {
        if (this.interactionData.isAnyInteractionActive()) {
            ...
            this.interactionData.cancelInteraction(this.context, EntityInteraction.InteractionResult.CANCELLED);
            PacketDistributor.sendToServer(C2S_RequestInteractionState.cancel(interaction.type(), this.entity));
        }
    }
    ...
    private static final class InteractionButton extends AbstractButton {
        ...
        private InteractStateCallback onInitiate = t -> {};
        private InteractStateCallback onCancel = t -> {};
        private InteractStateCallback onFinish = t -> {};
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
            ...
            guiGraphics.drawString(font, content, this.getX() + (this.width - contentWidth) / 2, this.getY() + (this.height - 8) / 2, this.active ? 0xFFFFFFFF : 0xFFAAAAAA);
